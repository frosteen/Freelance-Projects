...
##                        print("-----------------------------------------------------")
            ...
            for x in _5TimestampsAisData:
                if "x" in x and "y" in x and "sog" in x and "rot" in x:
                    ...
                    self.tableWidget.setItem(rowPosition, 2, QtGui.QTableWidgetItem(str(x["x"])))
                    self.tableWidget.setItem(rowPosition, 3, QtGui.QTableWidgetItem(str(x["y"])))
            ...
            mlp = MLPRegressor(hidden_layer_sizes=(100), 
                  activation='tanh', solver='lbfgs', alpha=1e-10)
            
            #Training
            scaler = StandardScaler()  # doctest: +SKIP
            # Don't cheat - fit only on training data
            scaler.fit(inputs)
            X_train = scaler.transform(inputs)  # doctest: +SKIP
            scaler.fit(outputs)  # doctest: +SKIP
            Y_train = scaler.transform(outputs)
...
##            X_test = scaler.transform(X_test)  # doctest: +SKIP
            ...
            for i in range(endTime,endTime+60*5, 1):
                ...
                xpts1.append(scaler.inverse_transform(predictionOutput)[0])
                ypts1.append(scaler.inverse_transform(predictionOutput)[1])
                trueHeadingAngles1.append(scaler.inverse_transform(predictionOutput)[5])
...
##                trueHeadingAngles1.append(predictionOutput[5])
            ...
            m.plot(xpts, ypts,'-',latlon=True, markersize=0, c="Blue", linewidth=1)
            xpts1.insert(0,xpts[-1])
            ypts1.insert(0,ypts[-1])
            m.plot(xpts1, ypts1,'-',latlon=True, markersize=0, c="Red", linewidth=1)
            ...
            for i in range(len(predictions)):
                ...
                self.tableWidget.setItem(rowPosition, 2, QtGui.QTableWidgetItem(str(predictions[i][0])))
                self.tableWidget.setItem(rowPosition, 3, QtGui.QTableWidgetItem(str(predictions[i][1])))
