...
class PesoImage:
    ...
    def SerialNumber(self):
        ...
        image_denoise = cv2.fastNlMeansDenoising(image_gray, None, 10, 7, 21)
        image_blur = cv2.medianBlur(image_denoise, 5)
        image_canny_edge = cv2.Canny(image_blur, 5, 5)
        image_thresh = cv2.threshold(image_canny_edge, 0, 255, cv2.THRESH_BINARY)[1]

        data = pytesseract.image_to_string(image_thresh)

        filtered_SN = data  # re.sub(r"\W+", "", data)

        len_filtered_SN = len(filtered_SN)

        # print("SN:", len_filtered_SN)

        if len_filtered_SN > 4:
            return image_thresh, True
    ...
    def SecurityThread(self):
        ...
        lines = cv2.HoughLines(image_canny_edge, 1, np.pi / 180, 40)
    ...
    def TemplateMatch(image, template):
        ...
        return minVal, maxVal, minLoc, maxLoc, startX, startY, endX, endY
