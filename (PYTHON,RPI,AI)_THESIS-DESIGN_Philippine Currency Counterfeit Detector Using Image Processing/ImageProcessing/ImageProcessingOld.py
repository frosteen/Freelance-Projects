...
class PesoImage:
    ...
    def SerialNumber(self):
        ...
        image_crop = self.image.copy()[
            int(self.h * 0.700) : int(self.h * 0.85),
        ...
        image_canny_edge = cv2.Canny(image_blur, 50, 200)
        image_thresh = cv2.threshold(image_canny_edge, 0, 255, cv2.THRESH_BINARY)[1]

        data = self.reader.readtext(image_blur, detail=0)

        print(data)

        filtered_SN = "asdfasqwe"  # re.sub(r"\W+", "", data)
