...
def peso_scanner(
    ...
    img_blur = cv2.GaussianBlur(img_denoise, (5, 5), 0)  # ADD GAUSSIAN BLUR
...
def initialize_trackbars(intialTracbarVals=0):
    ...
    cv2.createTrackbar("Threshold1", "Trackbars", intialTracbarVals, 255, nothing)
    cv2.createTrackbar("Threshold2", "Trackbars", intialTracbarVals, 255, nothing)


def get_val_tackbars():
...
if __name__ == "__main__":
    ...
    img = cv2.imread(
        "../Document_Scanner_Additional_12.15.2021/Test_Pictures/1000PHP.jpg"
    )

    H, W = img.shape[:2]

    initialize_trackbars(100)

    while True:
        thres1, thres2 = get_val_tackbars()  # GET TRACK BAR VALUES FOR THRESHOLDS

        peso_scanned = peso_scanner(img, int(W), int(Y * W / X), thres1, thres2, 10)

        h, w = peso_scanned.shape[:2]
        scale = 1
        cv2.namedWindow("Result", cv2.WINDOW_NORMAL)
        cv2.resizeWindow("Result", int(w * scale), int(h * scale))
        cv2.imshow("Result", peso_scanned)
