...
def numericalIntegration():
    for i in range(degree, -1, -1):
        coefX.insert(0, float(input("X{}: ".format(i))))
    n = int(input("n: "));
    a = float(input("a: "));
    b = float(input("b: "));
    ...
    i = a
    Headers = ["x", "F(x)", "F(X)&TRAPEOZIDAL PATTERN PRODUCT"]
    rows = []
    fxs = []
    num = 0
    cntr = 0
    while round(i, 10) <= b:
        if i == a or round(i,10) >= b: num = 1
        else: num = 2
    ...
    print("I =", round(h/2*sum(fxs), decimalPlace))
    print()

    #ROMBERG INTEGRATION
    i = 1
    Headers = ["n", "h", "I(h)", "O(h^2)", "O(h^4)", "O(h^6)", "O(h^8)"]
    rows = []
    iSagot = []
    en2 = []; en3 = []; en4 = []; en5 = []
    while i <= 16:
        ...
        cntr = 0
        while round(ii, 10) <= b:
            if ii == a or round(ii,10) >= b: num = 1
            else: num = 2
            fxsSum += num*eval(eqtn("ii"))
            ii += h
            cntr += 1
        rows.append([i, h, h/2*fxsSum])
        iSagot.append(h/2*fxsSum)
        i *= 2
    for i in range(4):
    ...
    for i in range(5):
        rows[i].append(en5[i])
    print("ROMBERG INTEGRATION (TRAPEZOIDAL RULE):")
    print(tabulate(rows, headers=Headers, tablefmt='plain', floatfmt='.{}f'.format(decimalPlace)))
    print()
    ...
    print(tabulate(rows, headers=Headers, tablefmt='plain', floatfmt='.{}f'.format(decimalPlace)))
    print("I =", round(h/3*sum(fxs), decimalPlace+1))
    print()
    ...
    while i <= 16:
        ...
        while round(ii, 10) <= b:
            ...
            ii += h
            cntr += 1
        rows.append([i, h, h/3*fxsSum])
        iSagot.append(h/3*fxsSum)
        i *= 2
    for i in range(4):
        en2.append(round(iSagot[i+1]+(1/(2**2-1)*(iSagot[i+1]-iSagot[i])), decimalPlace))
    en2.append("")
    ...
    for i in range(5):
        rows[i].append(en5[i])
    print("ROMBERG INTEGRATION (SIMPSONS 1/3 RULE):")
    print(tabulate(rows, headers=Headers, tablefmt='plain', floatfmt='.{}f'.format(decimalPlace)))
    print()
...
##            print(num)
    ...
    for i in range(5):
        rows[i].append(en5[i])
    print("ROMBERG INTEGRATION (SIMPSONS 3/8 RULE):")
    print(tabulate(rows, headers=Headers, tablefmt='plain', floatfmt='.{}f'.format(decimalPlace)))
    print()
