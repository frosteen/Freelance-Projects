        ...
        while True:
            ...
            if execute_status == True:
                ...
                for _, values in results_df.iterrows():
                    xmin, ymin, xmax, ymax, conf, _, name = values
                    if not abs(xmin - xmax) >= 40 or not abs(ymin - ymax) >= 50:
                        continue
                    cv2.rectangle(
                        frame,
                        (int(xmin), int(ymin)),
                    ...
                    name_conf = name + " " + str(round(conf, 2))
                    text_pos = (int(xmin), int(ymin - 10))
                    cv2.putText(
                        frame,
                        name_conf,
                        text_pos,
                    ...
                    class_name_location.append(
                        "-".join(self.check_location(frame, center) + [name])
                    )
