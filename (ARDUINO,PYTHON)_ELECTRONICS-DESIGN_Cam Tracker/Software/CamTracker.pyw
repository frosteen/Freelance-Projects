...
class MyWindow(QtGui.QMainWindow):
    ...
    def doText(self, text):
        ...
        self.th69 = DoThreading(run)
        self.th69.daemon = True
        self.th69.start()
            ...
            try:
                if self.arduino != None:
                    if self.arduino.isOpen():
                        self.arduino.close()
                self.serialCom = self.lineEdit.text()
                self.pushButtonCapture.setEnabled(False)
                ...
                self.pushButtonTrack.setEnabled(True)
                self.pushButtonStop.setEnabled(False)
                self.doText("Successfully connected!")
            except Exception as e:
                self.doText("Connection can't found. Restart Program...")
        doStart()
        self.arduino.write("A".encode()) #send character 'A' to arduino
        self.pushButtonTrack.setEnabled(False)
        self.pushButtonStop.setEnabled(True)
        self.pushButtonCapture.setEnabled(True)
        self.captureDevice = self.lineEdit_2.text()
