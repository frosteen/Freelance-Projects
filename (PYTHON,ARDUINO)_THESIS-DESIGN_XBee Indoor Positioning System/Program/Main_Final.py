...
readers = (
    ...
    Reader(
        "Reader3",
        # "0 13 A2 0 41 26 3B B7",
        # "0 13 A2 0 41 85 4B AB",
        "0 13 A2 0 41 DD 3D BE",
        (3.6, 2.0, 1.5),
        color="violet",
        A=-33,
        n=2.00,
...
try:
    ...
    port = serial.Serial(port=defined_port, baudrate=defined_baudrate)
...
class Window(QtWidgets.QMainWindow):
    ...
    def locate_tags(self):
        while True:
            ...
            reader_address, tag_address, rssi = [x[:-1] for x in data]

            for reader in readers:
                for tag in tags:
                    if reader_address == reader.address and tag_address == tag.address:
                        if tag.linked_readers_kalman_filter.get(reader.name) is None:
            ...
            for tag in tags:
                if len(tag.linked_readers_rssi) == len(readers):
                    for reader in readers:
                        filtered_rssi = tag.linked_readers_filtered_rssi[reader.name]
                        rssi = tag.linked_readers_rssi[reader.name]
                        distance = rssi_to_distance(filtered_rssi, reader.A, reader.n)
                        # distance = tag.linked_readers_rssi[reader.name]

                        tag.linked_readers_rssi[reader.name] = distance
                        with open(
                            os.getcwd()
                            + "/Reader-Tag-RSSI-FilteredRSSI-Distance-Timestamp.csv",
                            "a+",
                        ) as file:
                    ...
                    readers1, readers2, readers3 = readers[0], readers[1], readers[2]
                    ...
                    with open(os.getcwd() + "/Tag-X-Y-Timestamp.csv", "a+") as file:
                        dt_string = datetime.now().strftime("%d/%m/%Y %H:%M:%S")
                        file.write(
                            "{},{},{},{}\n".format(
                                tag.name,
            ...
            for tag1 in tags:
                for tag2 in tags:
                    if tag1.name != tag2.name:
                        ...
                        with open(
                            os.getcwd() + "/Tag-TagN-Distance-Timestamp.csv", "a+"
                        ) as file:
                            dt_string = datetime.now().strftime("%d/%m/%Y %H:%M:%S")
                            file.write(
                                "{},{},{},{}\n".format(
                                    tag1.name, tag2.name, tag_distance, dt_string
                                )
                            )
