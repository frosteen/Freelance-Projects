...
% Last Modified by GUIDE v2.5 27-Jan-2019 00:48:38
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
vi = str2num(getObj('edit1','string'));
qi = str2num(getObj('edit2','string'));
ci = str2num(getObj('edit3','string'));
qo = str2num(getObj('edit4','string'));
t1 = str2num(getObj('edit5','string'));
Qo = str2num(getObj('edit7', 'string'));
syms y(t) t;
v = vi + (qi-qo)*t;
ode = diff(y,t) == qi*ci - qo*(y/(v));
ySol = dsolve(ode,y(0) == Qo);
ans = double(subs(ySol, t, t1));
setObj('text8','string','Q = '+string(ans));

%PLOTTING%
data = [];
h = t1/100;
for x = 0:h:t1
   data = [data, double(subs(ySol, t, x))]; 
end
plot(0:h:t1,  data);
xlabel('t');
ylabel('Q');
